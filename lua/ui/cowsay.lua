...
--- wrap a string into cowsay bubble
-- @param text string the quote
-- @param width number maximum line length before wrap
function cowsay.cowsay_bubble(text, width)
    width = width or 80
    local lines = {}
    ...
    local out = {}
    table.insert(out, ' ' .. string.rep('_', maxlen + 2))

    if #lines == 1 then
        table.insert(out, '< ' .. lines[1] .. string.rep(' ', maxlen - #lines[1]) .. ' >')
    else
        for i, l in ipairs(lines) do
            local padding = string.rep(' ', maxlen - #l)
            if i == 1 then
                table.insert(out, '/ ' .. l .. padding .. ' \\')
            elseif i == #lines then
                table.insert(out, '\\ ' .. l .. padding .. ' /')
            else
                table.insert(out, '| ' .. l .. padding .. ' |')
    ...
    table.insert(out, ' ' .. string.rep('-', maxlen + 2))
...
function cowsay.generate()
    ...
    local animal_keys = vim.tbl_keys(cowsay.animals)
    local animal = cowsay.animals[animal_keys[math.random(#animal_keys)]]
